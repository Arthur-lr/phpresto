...
class TypeCuisineDAO {
    ...
    public static function suppressionParId(int $idTC): bool {
        $resultat = false;
        try {
            $requete = "DELETE FROM proposer WHERE idTC  = ?;"
                    . "DELETE FROM preferer WHERE idTC  = ?;"
                    . "DELETE FROM typeCuisine WHERE idTC  = ?;";
            
            
        $stmt = Bdd::getConnexion()->prepare($requete);
        $stmt->bindParam(1, $idTC, PDO::PARAM_INT);
        $stmt->bindParam(2, $idTC, PDO::PARAM_INT);
        $stmt->bindParam(3, $idTC, PDO::PARAM_INT);
    ...
    public static function addTypeCuisine(String $libelle){
        $ok = false;
        try {
            $requete = "INSERT INTO typeCuisine (libelleTC) VALUES (:libelleTC)";
            $stmt = Bdd::getConnexion()->prepare($requete);
            $stmt->bindValue(':libelleTC', $libelle);
            $ok = $stmt->execute();
        } catch (PDOException $e) {
    ...
    public static function update(TypeCuisine $unTC): bool {
        $ok = false;
        try {
        $requete = "UPDATE typeCuisine SET libelleTC = :libelle  WHERE idTC = :idTC";
        $stmt = Bdd::getConnexion()->prepare($requete);
        $stmt->bindValue(':libelle', $unTC->getLibelleTC());
        $stmt->bindValue(':idTC', $unTC->getIdTC());
