...
class RestoDAO {
    ...
    public static function update(Resto $unResto): bool {
        $ok = false;
        try {
        $requete = "UPDATE resto SET nomR = :nomR, numAdrR = :numAdrR, voieAdrR = :voieAdrR, cpR = :cpR, villeR = :villeR WHERE idR = :idR";
        $stmt = Bdd::getConnexion()->prepare($requete);
    ...
    public static function insert(Resto $unResto): bool {
        $resultat = false;
        try {
            $stmt = Bdd::getConnexion()->prepare("INSERT INTO resto (nomR,numAdrR,voieAdrR,cpR,villeR,descR) VALUES(?,?,?,?,?,?)");
            
            $stmt->bindParam(1, $unResto->getNomR());      
            $stmt->bindParam(2, $unResto->getNumAdr(), PDO::PARAM_INT);
            $stmt->bindParam(3, $unResto->getVoieAdr());
            $stmt->bindParam(4, $unResto->getCpR(), PDO::PARAM_INT);
            $stmt->bindParam(5, $unResto->getVilleR());
            $stmt->bindParam(6, $unResto->getDescR());
